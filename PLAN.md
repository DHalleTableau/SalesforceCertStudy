...
**Current progress (update this every time a sub-step finishes or is left mid-way):** Phase 4, sub-step **4d done and verified** (session setup now redirects to `/session/<id>/overview`; verified both the "pending" state before the worker has derived anything, and the real rendered duration/questions/passing score/domain breakdown after simulating what the worker would write). Next up: **4e** (the question loop -- serve/answer routes + grading).
