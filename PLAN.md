...
2. **Ingestion** — `ingest.py`: admin screen with two paste boxes for the sheet's `Exam Guides` and `Certification Prerequisites` tabs (pasted, not fetched — see Locked-in decisions). Parse `Exam Guides` into `resources` (role from Base/Extra) + `Cert` overview seed fields (`is_aggregate`, `detail_page_url`, `exam_guide_url`); parse `Certification Prerequisites` into `cert_prerequisites` edges. Tier-1 auto-fetch of individual Exam_Guide_URL content; Tier-2 paste fallback flagging for blocked pages. Verify: paste both tabs, confirm `resources` rows get correct primary/additional roles, `certs.is_aggregate` matches the sheet, `cert_prerequisites` edges match the PreReq columns (including a multi-level chain), a blocked Exam_Guide_URL gets flagged for paste.
3. **Worker + question generation** — `claude_client.py`, `worker.py`, cert-overview derivation from primary Exam Guide text, on-demand top-up loop. Verify: queue fills to ~5 `ready` rows before any question is served; long-form feedback intact.
4. **Question UI + review/export** — templates for setup/overview/question/review, question-serve and answer-recording routes, grading (multi all-or-nothing), review screen, CSV export. Verify: instant serve (no model call in request), correct grading, CSV matches right/wrong/both filter.
