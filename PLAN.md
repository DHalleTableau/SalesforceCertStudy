...
**Current progress (update this every time a sub-step finishes or is left mid-way):** Phase 4, sub-step **4c done and verified** (login → seed certs via `/admin/ingest` → `/session/setup` cert picker → prerequisite prompt answered "yes" → `StudySession` created with both certs resolved; also verified the no-prerequisites-needed immediate-create path). Next up: **4d** (overview route + template showing the resolved certs' derived duration/# questions/passing score/domain breakdown).
